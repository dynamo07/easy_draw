...
// show toast on one monitor only
...
static void LoadConfig() {
	...
	while (std::getline(f, line)) {
		...
		// customizable region screenshot hotkey
...
// area screenshot selection outline
...
// Draws the toast on all monitors (each bottom-center) or just one
...
// area screenshot completion post
...
// Capture a specific rectangle (screen coords), composite overlay, encode async
...
static LRESULT CALLBACK LowLevelKbProc(int nCode, WPARAM wParam, LPARAM lParam) {
	if (nCode == HC_ACTION) {
		...
		// Ctrl+S region screenshot (works in Draw/Text only; disabled in pass-through & magnifier)
	...
	switch (msg) {
		...
		case WM_TIMER:
			if (wParam == TOAST_TIMER_ID) {
				...
				g_toastVisible = false;
				g_toastOneMonitor = false; // reset to default (all monitors)
				RenderFrame(false);
				return 0;
			}
			break;
		...
		case WM_RBUTTONDOWN: {
			...
			// Right-click does not affect region screenshot state
			float x = (float)GET_X_LPARAM(lParam), y = (float)GET_Y_LPARAM(lParam);
			if (g_armStrokeAfterText) {
				g_armStrokeAfterText = false;
				ReleaseCapture();
