...
struct Style {
	...
	float minW  = 2.f;
	float maxW  = 40.f;   // default MAX 40 per requirements
	float stepW = 4.f;
	float width   = 6.f;  // current regular width
	float hiWidth = 60.f; // current highlight width
};
...
// Track previous interaction mode for Ctrl+2 round-trip
...
// Four previous sizes (global per mode, independent of color)
float g_prevRegularWidth   = 6.f;
float g_prevHighlightWidth = 60.f;
int   g_prevTextSize       = 36;
int   g_prevEraserSize     = 50;

// Text settings (one FONT line: family, spacing, min, max, step, [default])
int     g_fontMin     = 16;
int     g_fontMax     = 76;
int     g_fontStep    = 10;
...
// Preserve previous mode when entering text
bool g_prevEraser    = false;
bool g_prevHighlight = false;

// Arm drawing after quitting text by single LMB click (avoid dot)
bool           g_armStrokeAfterText = false;
D2D1_POINT_2F  g_armStart{0, 0};

// Eraser (ERASER_SIZE line: min max step [default])
int     g_eraserMin  = 20;
int     g_eraserMax  = 200;
int     g_eraserSize = 50;
int     g_eraserStep = 5;

// Mouse position for indicators
D2D1_POINT_2F g_mousePos{0, 0};
bool          g_haveMousePos = false;

// Highlight parameters
int   g_highlightAlpha = 50;            // 0..255
int   g_highlightWidthMultiple = 10;    // multiply regular min/max/step

// Remember last interactive mode across click-through toggle
...
ID2D1Bitmap1*        g_target = nullptr;      // swapchain backbuffer as D2D target
ID2D1Bitmap1*        g_contentBmp = nullptr;  // cached committed content
...
static void LoadConfig() {
	...
	if (!f.is_open()) {
		...
		add('G', 0.f, 0.78f, 0.f);
		add('B', 0.f, 0.47f, 1.f);
		// Make Y and O more distinct:
		add('Y', 1.f, 1.f, 0.f);         // pure yellow
		add('P', 1.0f, 105/255.f, 180/255.f);
		add('C', 0.f, 1.f, 1.f);
		add('V', 148/255.f, 0.f, 211/255.f);
		add('K', 0.f, 0.f, 0.f);
		add('W', 1.f, 1.f, 1.f);
		add('O', 1.f, 0.5f, 0.f);        // deeper orange
	...
	while (std::getline(f, line)) {
		...
		if (key == "COLOR") {
			...
			// ERASER_SIZE  min max step [default]
		...
		// Old per-font keys are ignored to keep config consistent.
	}
...
static void DrawSizeIndicator() {
	...
	if (g_textMode) {
		// Vertical line indicator with a small left gap from cursor X
		...
		if (g_eraser) {
			...
			d = g_highlight ? s.hiWidth : s.width;
			if (d > 0.f) {
				float r = d * 0.5f;
				D2D1_ELLIPSE e = D2D1::Ellipse(g_mousePos, r, r);
...
static void ToggleOverlay() {
	...
	if (!wasPass) {
		// Record current interactive mode
		if (g_textMode) {
		...
		// Finalize any in-progress stroke, but do NOT commit or cancel live text
		if (g_drawing) {
			g_drawing = false;
			if (!g_live.pts.empty()) {
				g_cmds.push_back(g_live);
		...
		// Keep the live text visible while in click-through
		if (g_textMode) {
			RenderFrame(true);
		} else {
			RenderFrame(false);
	...
	// Coming back from click-through: restore previous mode
	g_passThrough = false;
	ApplyPassThroughStyles();
...
static LRESULT CALLBACK LowLevelKbProc(int nCode, WPARAM wParam, LPARAM lParam) {
	if (nCode == HC_ACTION) {
		...
		// While typing, allow Ctrl + color to quit text mode and enter highlight of that color
...
static LRESULT CALLBACK OverlayWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_SETCURSOR:
		...
		case WM_LBUTTONDOWN: {
			...
			if (g_textMode) {
				// Quit text mode without drawing a dot on simple click
				CommitText();
