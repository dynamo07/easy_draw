...
// === NEW: track only one active pointer (first touch/pen) ===
...
// -------------------- Virtual desktop (multi-monitor) helper --------------------
...
static LRESULT CALLBACK LowLevelKbProc(int nCode, WPARAM wParam, LPARAM lParam) {
	if (nCode == HC_ACTION) {
		...
		// Disable 'M' toggle while typing text (pressing 'm' should just type)
...
static LRESULT CALLBACK OverlayWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_DISPLAYCHANGE:
		// Re-span the virtual desktop if monitors were added/removed or resolution changed
		...
		// === Low-latency pointer input path (touch/pen) ===
		case WM_POINTERDOWN: {
			if (g_passThrough) break;
			
			const UINT32 id = GET_POINTERID_WPARAM(wParam);
			
			// Only accept the first active pointer; ignore additional fingers
			if (g_activePointerId != 0) {
				return 0;
			}
		...
		case WM_POINTERUPDATE: {
			...
			// Use coalesced history (oldest -> newest) for smooth strokes
			UINT32 count = 0;
			if (!GetPointerInfoHistory(id, &count, nullptr) || count == 0) {
				...
				if (GetPointerInfo(id, &pi)) {
					...
					if (g_armStrokeAfterText) {
						float dx = fabsf(g_mousePos.x - g_armStart.x);
						float dy = fabsf(g_mousePos.y - g_armStart.y);
						if (dx >= 1.f || dy >= 1.f) {
							g_armStrokeAfterText = false;
			...
			if (GetPointerInfoHistory(id, &count, hist.data())) {
				...
				for (UINT32 i = 0; i < count; ++i) {
					...
					if (g_armStrokeAfterText) {
						float dx = fabsf(p.x - g_armStart.x);
						float dy = fabsf(p.y - g_armStart.y);
						if (dx >= 1.f || dy >= 1.f) {
							g_armStrokeAfterText = false;
							BeginStroke(g_armStart.x, g_armStart.y);
							AddToStroke(p.x, p.y);
				...
				if (g_magnify) {
					...
					if (g_textMode) RenderFrame(true); else RenderFrame(false);
				}
			}
			return 0;
		} break;
		...
		// Extra safety: end stroke if the active pointer leaves/cancels or capture changes
		...
		case WM_MOUSEMOVE: {
			// Ignore legacy mouse path while a pointer is active (prevents double-processing)
			...
			// Safety: if somehow we were drawing but the mouse button isn't down, end the stroke
		...
		case WM_LBUTTONDOWN: {
			// Ignore if pointer path is active
			if (g_activePointerId != 0) return 0;
			if (g_passThrough) {
				break;
			}
	...
	case WM_CAPTURECHANGED:
		...
		g_armStrokeAfterText = false;
		// If capture changed while a pointer was active, clear it
		g_activePointerId = 0;
		return 0;
		
	case WM_LBUTTONUP:
		// Ignore if pointer path handled it
		if (g_activePointerId != 0) return 0;
		if (g_passThrough) {
			break;
		}
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int) {
	...
	// Use virtual desktop geometry (all monitors)
	int vx = GetSystemMetrics(SM_XVIRTUALSCREEN);
	int vy = GetSystemMetrics(SM_YVIRTUALSCREEN);
	int vw = GetSystemMetrics(SM_CXVIRTUALSCREEN);
	int vh = GetSystemMetrics(SM_CYVIRTUALSCREEN);
	...
	// Disable touch visual feedback & common gestures for low latency
	{
		BOOL off = FALSE;
	...
	// Make sure swap chain matches the actual client size
	ResizeToVirtualDesktop();
