...
int   g_w = 0;
int   g_h = 0;
int   g_vx = 0; // virtual desktop origin x
int   g_vy = 0; // virtual desktop origin y
...
// Track only one active pointer (first touch/pen)
...
static void DrawMagnifierWindowOutline() {
	...
	// Adjust from screen coords to overlay's client coords (virtual desktop origin)
	const float offx = (float)g_vx;  
	const float offy = (float)g_vy;   
	
	const float expand = 3.0f;
	D2D1_RECT_F rcOuter = D2D1::RectF(
		(FLOAT)wr.left   - offx - expand,
		(FLOAT)wr.top    - offy - expand,
		(FLOAT)wr.right  - offx + expand,
		(FLOAT)wr.bottom - offy + expand
		);
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int) {
	...
	// Ensure swap chain matches the actual client size
	ResizeToVirtualDesktop();
