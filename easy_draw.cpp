...
static void UpdateMagnifierPlacementAndSource(bool force = false);
...
static void EnsureMagnifierWindow() {
	...
	MAGTRANSFORM mt{};
	mt.v[0][0] = (float)g_magLevel;
	mt.v[1][1] = (float)g_magLevel;
	mt.v[0][1] = 0.0f;
	mt.v[1][0] = 0.0f;
	mt.v[0][2] = 0.0f;
	mt.v[1][2] = 0.0f;
...
static void UpdateMagnifierPlacementAndSource(bool force) {
	...
	if (force || desiredLeft != g_magPrevLeft || desiredTop != g_magPrevTop ||
	...
	bool zoomChanged = (g_magPrevZoom != g_magLevel);
...
static LRESULT CALLBACK LowLevelKbProc(int nCode, WPARAM wParam, LPARAM lParam) {
	if (nCode == HC_ACTION) {
		...
		if (isKeyDown && up == g_keyMagnify && !g_passThrough) {
			if (g_magnify) {
				g_magnify = false;
	...
	if (nCode == HC_ACTION) {
		...
		if (wParam == WM_MOUSEWHEEL) {
			...
			if (g_magnify && g_magHasRect) {
				...
				UpdateMagnifierPlacementAndSource(true);
				
				if (g_hMag) {
					InvalidateRect(g_hMag, nullptr, FALSE);
					UpdateWindow(g_hMag);
				}
				if (g_hMagHost) {
					InvalidateRect(g_hMagHost, nullptr, FALSE);
					UpdateWindow(g_hMagHost);
				}
		...
		case WM_MOUSEMOVE: {
			...
			if (g_magnify) {
				if (g_magSelecting) {
					...
					UpdateMagnifierPlacementAndSource(false);
					RenderFrame(false);
					return 0;
				}
			}
