...
// Text edit undo/redo stacks (live typing only)
vector<wstring> g_textUndoStack;
vector<wstring> g_textRedoStack;

// Eraser (ERASER_SIZE: min max step [default])
int     g_eraserMin  = 20;
int     g_eraserMax  = 200;
int     g_eraserSize = 50;
int     g_eraserStep = 5;
...
static LRESULT CALLBACK LowLevelKbProc(int nCode, WPARAM wParam, LPARAM lParam) {
	if (nCode == HC_ACTION) {
		...
		// In text mode: Ctrl+Z (undo typing), Ctrl+A (redo typing), Ctrl+<color> switch to highlight
